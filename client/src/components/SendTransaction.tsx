...
const SendTransaction: React.FC = () => {
  ...
  const senderAddress = useSelector<RootState, string>(
  ...
  const {
    ...
    register,
    formState: { errors },
  } = useForm<SendTransactionFormState>({
    defaultValues: {
      senderAddress,
      recipientAddress: "",
    },
  ...
  const isFormValid = Object.values(errors).length === 0;

  const onSubmit = useCallback(
    (formData: SendTransactionFormState) => {
      if (!isFormValid) {
        return;
      }
  ...
  // this can be simplified by a toggle function that accepts a boolean
  // but a small repetition in the name of explicitness is just fine for this case
              ...
              <div className="bg-white border shadow-sm rounded-xl w-modal">
                ...
                <div className="p-4 overflow-y-auto">
                  ...
                  <input
                    type="text"
                    id="input-recipient"
                    className="opacity-70 py-3 px-4 block border-gray-800 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 w-full"
                  ...
                  {errors.recipientAddress && (
                    <span className="text-amber-900 font-[14px]">
                ...
                <div className="flex justify-end items-center gap-x-2 py-3 px-4 border-t">
                  ...
                  >
                    {transactionModal.loading ? "Sending..." : "Send"}
                  </button>
                </div>
              </div>
            </div>
